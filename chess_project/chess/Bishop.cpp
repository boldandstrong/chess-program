...
int Bishop::isMoveValid(string dest) const
{
	...
	if (diffX < 0 && diffY < 0)
	{
		for (i = srcIndex[0], j = srcIndex[1]; i < destIndex[0] && j < destIndex[1]; i++, j++)
		{
	...
	{
		for (i = destIndex[0], j = srcIndex[1]; i < srcIndex[0] && j < destIndex[1]; i++, j++)
		{
			if (this->_board->_board[i][j] != nullptr)
			{
				delete[] srcIndex;
	...
	{
		for (i = srcIndex[0], j = destIndex[1]; i < destIndex[0] && j < srcIndex[1]; i++, j++)
		{
			if (this->_board->_board[i][j] != nullptr)
			{
				delete[] srcIndex;
